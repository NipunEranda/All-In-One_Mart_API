...
async function registerCustomer(req, res) {
    try {
        ...
        var result = await config.executeQuery('CALL customer_Registration("' + req.body.firstName + '", "' + req.body.lastName + '", "' + req.body.dob + '", "' + req.body.phoneNo + '", "' + req.body.gender + '", "' + req.body.address + '", "' + req.body.email + '", "' + encryptedPassword + '", "' + code + '", @output); SELECT @output as output;');
...
async function registerSupplier(req, res) {
    try {
        ...
        let code = helper.generateString(6);
        encryptedPassword = await bcrypt.hash(req.body.password, 10);
        var result = await config.executeQuery('CALL supplier_Registration("' + req.body.firstName + '", "' + req.body.lastName + '", "' + req.body.dob + '", "' + req.body.phoneNo + '", "' + req.body.gender + '", "' + req.body.address + '", "' + req.body.email + '", "' + encryptedPassword + '", "' + code + '", "' + req.body.joinedDate + '", @output); SELECT @output as output;');
        const output = helper.processProcedureOutput(res, result, "Something went wrong in registration.");
        if (output.code == 200) {
            //Send the generated code via mail
            helper.sendEmail(req, helper.getEmailTemplate(1, { code: req.body.email + '+' + code }).subject, helper.getEmailTemplate(1, { code: req.body.email + '+' + code }).body);
